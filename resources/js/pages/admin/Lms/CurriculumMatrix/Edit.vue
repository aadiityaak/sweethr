...
interface MatrixItem {
    ...
    lms_material_id: number | null;
    lms_quiz_id: number | null;
...
const form = useForm({
    ...
    lms_category_id: props.item.lms_category_id ?? '',
    lms_material_id: props.item.lms_material_id ?? '',
    lms_quiz_id: props.item.lms_quiz_id ?? '',
    lms_assignment_id: props.item.lms_assignment_id ?? '',
    is_mandatory: props.item.is_mandatory,
    deadline_days: props.item.deadline_days ?? 30,
});
...
const submit = () => form.put(`/admin/curriculum-matrix/${props.item.id}`);
</script>

<template>
    <Head title="Edit Item Kurikulum" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto max-w-2xl space-y-6 p-6">
            <div class="flex items-center gap-3">
                ...
                <h1 class="text-2xl font-bold text-gray-900">Edit Item Kurikulum</h1>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-5 rounded-lg border bg-white p-6">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Tipe Item</label>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Batas Waktu (hari)</label>
