...
import { AlertCircle, Camera, Check, Loader2, X } from 'lucide-vue-next';
...
const loadModels = async () => {
    try {
        ...
        const loadWithTimeout = async (loadFunction: () => Promise<void>, name: string, timeoutMs = 15000) => {
            ...
            return Promise.race([
                loadFunction(),
                new Promise((_, reject) => setTimeout(() => reject(new Error(`${name} loading timeout after ${timeoutMs}ms`)), timeoutMs)),
            ]);
        };

        await loadWithTimeout(() => faceapi.nets.tinyFaceDetector.loadFromUri(MODEL_URL), 'tiny face detector');

        await loadWithTimeout(() => faceapi.nets.faceLandmark68Net.loadFromUri(MODEL_URL), 'face landmarks');

        await loadWithTimeout(() => faceapi.nets.faceRecognitionNet.loadFromUri(MODEL_URL), 'face recognition');
...
const startFaceDetection = () => {
    ...
    detectionInterval.value = window.setInterval(async () => {
        ...
        try {
            ...
            const detection = await faceapi.detectSingleFace(videoRef.value, detectionOptions).withFaceLandmarks().withFaceDescriptor();
...
<template>
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
        <div class="relative mx-4 w-full max-w-lg rounded-xl bg-white shadow-2xl dark:bg-gray-900">
            <!-- Header -->
            ...
            <div class="p-4">
                <div class="mb-6 flex justify-center">
                    <div
                        class="relative aspect-square h-56 w-56 overflow-hidden rounded-full border-4 bg-gray-900 sm:h-72 sm:w-72 md:h-80 md:w-80"
                        :class="[faceDetected ? 'border-green-500' : 'border-gray-300']"
                    >
                        <!-- Loading State -->
                        <div v-if="isLoading" class="absolute inset-0 flex items-center justify-center bg-gray-900/50">
                            <div class="rounded-lg bg-gray-800/80 p-6 text-center text-white">
                        ...
                        <div v-if="!isLoading && faceDetected" class="absolute inset-0 overflow-hidden rounded-full">
                            <!-- Vertical Scanning Line -->
                            <div class="absolute inset-0">
                                <div
                                    class="animate-scan absolute right-0 left-0 h-1 bg-gradient-to-r from-transparent via-white to-transparent opacity-80 shadow-lg"
                                ></div>
                            </div>

                            <!-- Pulsing Rings -->
                            <div class="absolute inset-2 animate-pulse rounded-full border-2 border-green-400/30"></div>
                            <div class="absolute inset-6 animate-ping rounded-full border border-green-400/20" style="animation-duration: 2s"></div>

                            <!-- Corner Markers -->
                            <div class="absolute top-8 left-8 h-6 w-6 rounded-tl-lg border-t-2 border-l-2 border-green-400 opacity-60"></div>
                            <div class="absolute top-8 right-8 h-6 w-6 rounded-tr-lg border-t-2 border-r-2 border-green-400 opacity-60"></div>
                            <div class="absolute bottom-8 left-8 h-6 w-6 rounded-bl-lg border-b-2 border-l-2 border-green-400 opacity-60"></div>
                            <div class="absolute right-8 bottom-8 h-6 w-6 rounded-br-lg border-r-2 border-b-2 border-green-400 opacity-60"></div>
                        </div>

                        <!-- Face Detection Indicator - 30px from top, centered horizontally -->
                        <div class="pointer-events-none absolute top-8 left-1/2 -translate-x-1/2 transform">
                            <div class="space-y-2 text-center">
                                ...
                                >
                                    <div
                                        ...
                                        :class="{
                                            'animate-pulse bg-green-500': faceDetected,
                                            'bg-red-500': !faceDetected && !isLoading,
                                            'animate-spin bg-gray-500': isLoading,
                                        }"
                                    ></div>
                                ...
                                <div
                                    v-if="mode === 'verification' && verificationConfidence !== null"
                                    class="inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-medium backdrop-blur-sm transition-all duration-300"
                        ...
                        <div
                            v-if="mode === 'setup' && capturedDescriptors.length > 0"
                            class="pointer-events-none absolute bottom-8 left-1/2 -translate-x-1/2 transform"
                        >
                            <div
                                class="flex items-center gap-2 rounded-full bg-blue-100/80 px-4 py-2 text-sm font-medium text-blue-800 backdrop-blur-sm dark:bg-blue-900/40 dark:text-blue-300"
                            >
                                <Check class="h-4 w-4" />
                ...
                <div v-if="mode === 'setup'" class="mt-4 text-center">
                    <div
                        class="inline-flex items-center gap-2 rounded-full bg-blue-50/80 px-4 py-2 text-sm font-medium text-blue-700 backdrop-blur-sm dark:bg-blue-900/20 dark:text-blue-300"
                    >
                ...
                <div class="mt-4 flex justify-center">
                    ...
                    >
                        ...
                        {{
                            isProcessing
                                ? 'Memproses...'
                                : mode === 'verification' && verificationConfidence !== null
                                  ? 'Verifikasi Ulang'
                                  : mode === 'setup'
                                    ? 'Ambil Foto'
                                    : 'Verifikasi Wajah'
                ...
                <div
                    v-if="errorMessage"
                    class="mt-4 flex items-center gap-2 rounded-lg bg-red-50 p-3 text-sm text-red-800 dark:bg-red-900/20 dark:text-red-400"
                >
                ...
                <div v-if="mode === 'verification' && verificationConfidence !== null" class="mt-4">
                    <div class="rounded-lg border border-green-200 bg-green-50 p-4 dark:border-green-800 dark:bg-green-900/10">
                        ...
                        <div class="mt-2 text-xs text-green-600 dark:text-green-400">Wajah terdeteksi dan absensi dapat dilanjutkan.</div>
                    </div>
                </div>
            </div>
        </div>
